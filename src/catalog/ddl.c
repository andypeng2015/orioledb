...
set_toast_oids_and_compress(Relation rel, Relation toast_rel)
{
	...
	o_tables_after_update(o_table, oxid, &o_snapshot);
...
transientrel_startup(DestReceiver *self, int operation, TupleDesc typeinfo)
{
	...
	fill_current_oxid_osnapshot(&myState->oxid, &myState->snapshot);
}
...
transientrel_receive(TupleTableSlot *slot, DestReceiver *self)
{
	...
	o_tbl_insert(myState->descr, myState->rel, slot, myState->oxid, &myState->snapshot);
...
drop_table(ORelOids oids)
{
	...
	table = o_tables_drop_by_oids(oids, oxid, &o_snapshot);
...
rewrite_table(Relation rel, OTable *old_o_table, OTable *new_o_table)
{
	...
	while (!O_TUPLE_IS_NULL(tup = btree_seq_scan_getnext(sscan, old_slot->tts_mcxt, &tuple_o_snapshot, &hint)))
	{
		tts_orioledb_store_tuple(old_slot, tup, old_descr,
								 &temp_o_snapshot, PrimaryIndexNumber,
								 true, &hint);
		...
		o_tbl_insert(descr, rel, new_slot, oxid, &o_snapshot);
							...
							int subId, void *arg)
{
	...
	else if (access == OAT_DROP && classId == DatabaseRelationId)
	{
		...
		o_tables_table_meta_lock(NULL);
		o_tables_drop_all(oxid, &o_snapshot, objectId);
		o_tables_table_meta_unlock(NULL, InvalidOid);
	}
			 ...
			 ActiveSnapshotSet())
	{
		...
		o_tables_drop_columns_by_type(oxid, &o_snapshot, objectId);
			...
			else if (rel->rd_rel->relkind == RELKIND_INDEX)
			{
					...
					is_orioledb_rel(tbl))
				{
					...
					else
					{
						...
						o_tables_update(o_table, oxid, &o_snapshot);
						o_indices_update(o_table, ix_num, oxid, &o_snapshot);
